...
console.log('[Email] SMTP:',mailTransporter?'READY (real delivery)':'DEMO MODE (in-app follow-up only)');
const app=express();
...
app.get('/api/dashboard',auth(),safe(async(req,res)=>{const t=await Trainee.find();const total=t.length, outcomes=t.filter(x=>['employed','self_employed','apprenticeship'].includes(x.status));const placed=outcomes.length;const wage=t.filter(x=>x.wage>0).map(x=>x.wage);const verified=t.filter(x=>x.employerVerified).length;const reasons={};t.forEach(x=>{const r=x.nonPlacementReason||x.attritionReason;if(r)reasons[r]=(reasons[r]||0)+1});const courses={},districts={};t.forEach(x=>{for(const [key,val] of [['courses',x.course],['districts',x.district]]){};const target=courses[x.course]||(courses[x.course]={total:0,outcomes:0});target.total++;if(['employed','self_employed','apprenticeship'].includes(x.status))target.outcomes++;const d=districts[x.district]||(districts[x.district]={total:0,outcomes:0});d.total++;if(['employed','self_employed','apprenticeship'].includes(x.status))d.outcomes++;});const gaps={};t.forEach(x=>(x.skillGap||[]).forEach(g=>gaps[g]=(gaps[g]||0)+1));res.json({total,consented:t.filter(x=>x.consent).length,certified:t.filter(x=>x.trainingStatus==='certified'||x.status==='certified').length,placed,placementRate:total?Math.round(placed/total*100):0,avgWage:wage.length?Math.round(wage.reduce((a,b)=>a+b,0)/wage.length):0,verified,verificationRate:placed?Math.round(verified/placed*100):0,statuses:{employed:t.filter(x=>x.status==='employed').length,selfEmployed:t.filter(x=>x.status==='self_employed').length,apprenticeship:t.filter(x=>x.status==='apprenticeship').length,jobSearch:t.filter(x=>x.status==='job_search').length,furtherEducation:t.filter(x=>x.status==='further_education').length,droppedOut:t.filter(x=>x.status==='dropped_out').length,inTraining:t.filter(x=>x.status==='training').length,certifiedAwaiting:t.filter(x=>x.status==='certified').length},retention:{month3:t.filter(x=>x.retention?.month3).length,month6:t.filter(x=>x.retention?.month6).length,month12:t.filter(x=>x.retention?.month12).length},courses,districts,gaps,reasons,journey:{enrolled:total,certified:t.filter(x=>x.trainingStatus==='certified'||x.status==='certified').length,placed,retained6:t.filter(x=>x.retention?.month6).length,retained12:t.filter(x=>x.retention?.month12).length}});}));
...
app.patch('/api/trainee/me/profile',auth(['trainee']),safe(async(req,res)=>{const allowed=['name','phone','district','demographics'];const patch={};allowed.forEach(k=>{if(req.body[k]!==undefined)patch[k]=req.body[k]});const t=await Trainee.findByIdAndUpdate(req.user.linkedId,patch,{new:true});if(req.body.name)await User.findByIdAndUpdate(req.user.id,{name:req.body.name});res.json(t);}));
...
app.patch('/api/trainee/me/outcome',auth(['trainee']),safe(async(req,res)=>{
  ...
  const patch={lastOutcomeUpdate:new Date().toISOString().slice(0,10)};
...
app.post('/api/trainees/:id/certificate',auth(['admin','provider']),safe(async(req,res)=>{const t=await Trainee.findById(req.params.id);if(!t)return res.status(404).json({message:'Trainee not found'});if(req.user.role==='provider'){const p=await Provider.findById(req.user.linkedId);if(t.provider!==p?.name)return res.status(403).json({message:'You can only certify your own trainees'});}
 ...
 const certificateId=t.certificateId||`DISHA-CERT-${new Date().getFullYear()}-${t.traineeId}`;t.certificateId=certificateId;t.trainingStatus='certified';if(t.status==='training')t.status='certified';t.assessmentScore=avg;await t.save();await Enrollment.updateMany({traineeId:t.traineeId,courseTitle:t.course},{status:'certified',completedAt:new Date(),certificateId});res.json({message:'Certificate issued successfully',certificateId,average:avg});
}));
